...
def finalize_and_save(posts, blog_id, logger, sort_by="likes"):
    ...
    if not posts:
        logger.warning("⚠️ 수집 범위 내에 데이터가 없습니다.")
        return []
    ...
    logger.info(f"✨ 분석 성공: {len(sorted_posts)}개 저장 ({sort_label} 정렬)")
    logger.info(f"📍 위치: {file_path}")
    return sorted_posts
...
def scroll_and_collect(blog_id, mode="all", target_count=0, start_date_str=None, end_date_str=None, sort_by="likes", progress_callback=None, stop_event=None):
    ...
    logger.info(f"🚀 분석을 시작합니다. (블로그: {blog_id})")
    ...
    while True:
        ...
        api_url = f"https://m.blog.naver.com/api/blogs/{blog_id}/post-list?categoryNo=0&itemCount=24&page={page}"
        
        try:
            response = requests.get(api_url, headers=headers, timeout=10)
            if response.status_code != 200:
                logger.error(f"❌ 네이버 응답 실패 (코드: {response.status_code})")
                break
            ...
            if not items: 
                logger.info("🏁 마지막 글까지 확인했습니다.")
                break
            
            current_date = datetime.fromtimestamp(items[0].get("addDate") / 1000.0).strftime("%Y-%m-%d")
            logger.info(f"📄 {page}페이지 분석 중... (게시일: {current_date})")

            for item in items:
                dt = datetime.fromtimestamp(item.get("addDate") / 1000.0)
                date_only = datetime(dt.year, dt.month, dt.day)

                if mode == "date":
                    if date_only > end_date: continue
                    if date_only < start_date:
                        logger.info(f"🏁 목표 기간 완료 ({start_date.strftime('%Y-%m-%d')} 이전 글 도달)")
...
"""
================================================================================
[ 네이버 블로그 포스트 분석기 개발 히스토리 및 기술 결정 사유 ] 지우지 말 것!!
================================================================================

...
def start_crawling():
    ...
    mode = mode_var.get()
    sort_by = sort_var.get() # 정렬 기준 가져오기
...
tk.Radiobutton(sort_frame, text="좋아요 수", variable=sort_var, value="likes").pack(side=tk.LEFT)
tk.Radiobutton(sort_frame, text="댓글 수", variable=sort_var, value="comments").pack(side=tk.LEFT)
...
description_text.insert(tk.END, "- 수집이 완료되면 결과 파일 상단에 '수집 시점'이 기록됩니다.\n")
description_text.insert(tk.END, "- '댓글 수' 정렬을 선택하면 가장 반응이 뜨거운 글부터 보여줍니다.\n\n")
description_text.insert(tk.END, "⚠️ 저장 위치: " + os.path.dirname(os.path.abspath(__file__)))
description_text.config(state=tk.DISABLED)
description_text.grid(row=5, column=0, columnspan=2, padx=15, pady=5, sticky="nsew")

# 진행 상태
progress_label = tk.Label(root, text="준비 완료")
progress_label.grid(row=6, column=0, columnspan=2, pady=(10, 0))
